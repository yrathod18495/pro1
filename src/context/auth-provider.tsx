...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const updatePresence = (status: 'online' | 'offline') => {
        ...
        // so tab-focus/reconnect churn doesn't fire extra writes — the
        // underlying write is idempotent anyway, this just keeps volume down.
        if (status === 'online' && typeof window !== 'undefined') {
            try {
