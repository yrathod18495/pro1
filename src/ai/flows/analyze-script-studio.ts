...
export async function analyzeScriptStudio(input: { script: string, userId: string, userEmail?: string, includeEmotion?: boolean }): Promise<any> {
  ...
  // that, analysis doesn't just stop — it becomes a paid action: 300
  // credits per analysis, charged right here via a transaction (same
  // pattern as every other credit deduction in this app), same as the
  // free-tier check above but for the analysis itself instead of the
  // eventual generation.
  ...
  if (overDailyLimit) {
    ...
    if (database) {
      await database.ref(`creditHistory/${input.userId}`).push({
        amount: -OVER_LIMIT_ANALYSIS_COST,
  ...
  try {
    ...
    return { 
      ...
      maxDailyLimit,
      creditsCharged: overDailyLimit ? OVER_LIMIT_ANALYSIS_COST : 0
    };
  } catch (error: any) {
