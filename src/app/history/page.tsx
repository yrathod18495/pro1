...
function HistoryPageContent() {
  ...
  //
  // 🐛 The partitioned queries below used to have NO orderBy — just a bare
  // `limit()`. Without an explicit order, Firestore does not guarantee
  // "most recent first"; in practice it falls back to ascending document
  // ID order. Once an account has more completed projects than
  // `itemsLimit * 2`, that silently cuts off newly created ones instead of
  // old ones — a brand-new 11Labs project can generate successfully,
  // reach "completed" in Firestore, and still never appear here, because
  // it fell outside the arbitrary slice the query happened to return.
  // Each partitioned query below now orders explicitly (desc) so recent
  // items are what the first page actually returns — see each query for
  // which field it orders by and why.
  ...
  // Partitioned queries are scoped by path (no `where`), so adding
  // orderBy here needs no extra Firestore index — it's safe and free.
  //
  // The 'projects' collection specifically is shared by several different
  // writers (voice studio, script generator, store purchases, voice
  // cloning) and not all of them stamp the same `timestamp` field on
  // every doc — ordering by it here would just swap "hides new voice
  // projects" for "silently hides script/store projects that lack
  // `timestamp`". Document-ID ordering has no such risk: every document
  // has an ID, so nothing can be excluded by missing a field. IDs here
  // are `HQ_<epoch-ms>_<random>` / `FG_<random>` / `job_<epoch-ms>_...`,
  // so descending ID order still surfaces the newest project within its
  // own prefix first — which is exactly the case that was breaking.
  const partitionedProjectsQuery = useMemoFirebase(() => {
      if (authLoading || !firestore || !activeUid) return null;
      // 🔴 FIX: this used to be `orderBy(documentId(), 'desc')` with
      // `limit(itemsLimit * 2)`. Document IDs in this collection come in
      // TWO different formats — old ones like "HQ_1784465488398_232TR3"
      // (starts with a digit) and newer ones like "HQ_AL14_1784795402047"
      // (starts with a letter). In ASCII, digits sort BELOW letters, so a
      // descending document-ID order puts every letter-prefixed ID first
      // — and once the limit is hit, newer digit-prefixed HQ projects
      // never make it into the result at all, even though they exist in
      // Firestore. That's why HQ Studio projects were silently missing
      // from /history. No single Firestore-level ordering is safe across
      // both ID shapes, so this fetches WITHOUT ordering/limiting at the
      // query level — correctness over a smaller read — and leaves
      // sorting to the existing client-side `combined.sort(...)` below,
      // which already merges + sorts every collection together anyway.
      return query(
          collection(firestore, 'projects', activeUid, 'userProjects')
      );
  }, [firestore, activeUid, authLoading]);
  ...
  }, [rootProjects, rootProProjects, rootMusicProjects, rootScriptProjects, partitionedProjects, proProjects, musicPartitionedProjects, scriptProjects, thumbnails, partitionedThumbnails, snakeCaseThumbnails, rootThumbnails]);
