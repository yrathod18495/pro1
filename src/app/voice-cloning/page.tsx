...
export default function VoiceCloningPage() {
    ...
    const handleGeneration = async () => {
        ...
        try {
            ...
            toast({ title: 'Cloning Started', description: 'Your voice worker has received the request.' });
    ...
    const togglePlay = (item: HistoryItem) => {
        ...
        if (playingId === item.id) { playRequestId.current++; player.pause(); setPlayingId(null); }
        else {
            player.pause();
            ...
            if (url) {
                ...
                player.play().then(() => {
                    // Ignore stale resolutions from a play() call that's no longer
                    // the latest one (user already switched to a different item).
                    if (playRequestId.current === requestId) setPlayingId(item.id);
                }).catch((err) => {
                    // Rapidly switching tracks calls pause() on the previous play()
                    // request before it settles, which the browser reports as an
                    // AbortError ("interrupted by a call to pause()"). This is
                    // expected behavior, not an app bug — only report/toast when
                    // it's a genuine playback failure.
    ...
    const handleDeleteHistory = async (id: string) => {
        ...
        if (playingId === id) { playRequestId.current++; audioRef.current?.pause(); setPlayingId(null); }
