 ...
 * src/app/voice-cloning/page.tsx.
 * Generation now happens on the HF Space worker (server-files/voice_cloning.py):
 * the page writes a job to RTDB `voice_cloning/{jobId}` and this action + the
 * dispatcher below it are never invoked. Safe to delete once you've confirmed
 * the new flow is stable in production.
 */
