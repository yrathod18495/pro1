...
function AnalysisExecutionModeSettings() {
    ...
    // on the Hugging Face backend.
    const [mode, setMode] = useState<'realtime' | 'server'>('server');
...
export default function AdminPage() {
  ...
  const adminTools = [
    ...
    { id: '11labs-studio', label: '11Labs Studio', icon: <Sparkles className="h-3 w-3 text-violet-500" /> },
    ...
    <div className="space-y-6 pb-20 max-w-7xl mx-auto w-full px-1">
      ...
      <div className="grid gap-6 grid-cols-1 md:grid-cols-3">
       ...
       <Tabs defaultValue="overview" className="w-full">
        {/* Tab bar. Rebuilt for both screens: each tab is an icon plus a
            plain, self-explanatory label (the old "Master / Neural /
            Assets" gave no idea what they did). On a phone the row scrolls
            horizontally with a VISIBLE fade so it's obvious there's more,
            and the touch targets are full-height. On desktop it simply
            centers. The sticky offset is measured from a CSS var instead
            of a hardcoded 104px that only matched one header height. */}
        ...
        <TabsContent value="overview" className="mt-4 space-y-6">
          <div className="grid gap-6 lg:grid-cols-3">
            ...
            <Card className="rounded-[2.5rem] border-none shadow-2xl bg-card overflow-hidden h-fit">
                <CardHeader className="bg-primary/5 p-6 border-b border-primary/10"><CardTitle className="text-[10px] font-black uppercase tracking-[0.4em] flex items-center gap-3"><Wrench className="h-5 w-5 text-primary" />Tool Access</CardTitle></CardHeader>
                <CardContent className="p-3">
                    {/* Tool toggles. One column on a phone (two half-width
                        rows were cramped and the labels truncated), two on
                        wider screens. */}
                    <div className="grid grid-cols-2 gap-2">
                        {toolSettingsLoading ? <Skeleton className="h-64 col-span-2 rounded-[2rem]" /> : adminTools.map((tool) => {
                            ...
                            return (
                                <div key={tool.id} className={cn("flex items-center justify-between p-2.5 rounded-xl border transition-all duration-300", isLocked ? "bg-destructive/5 border-destructive/10" : "bg-background border-primary/5 shadow-sm")}>
                                    <div className="flex items-center gap-2 min-w-0">
                                        ...
                                        <div className="min-w-0"><p className="text-[10px] font-black uppercase truncate">{tool.label}</p><p className={cn("text-[8px] font-bold uppercase", isLocked ? 'text-destructive' : 'text-green-600')}>{isLocked ? 'OFFLINE' : 'LIVE'}</p></div>
                                    </div>
                                    <Switch checked={!isLocked} onCheckedChange={() => toggleTool(tool.id, isLocked)} className="scale-75 origin-right data-[state=checked]:bg-green-600 data-[state=unchecked]:bg-destructive" />
                                </div>
                            );
                        })}
                    </div>
