...
import './globals.css';
// 🎬 Motion system — the landing page's anim-* library. Imported AFTER
// globals.css on purpose: these are plain CSS rules (no @layer, since
// Next processes each CSS file separately), so cascade order is what
// gives them precedence over Tailwind's utilities.
import './motion.css';
...
export const viewport: Viewport = {
  ...
  maximumScale: 1,
  userScalable: false,
  // Lets the page draw edge-to-edge (behind the status bar / notch) and
  // exposes env(safe-area-inset-top) so we can deliberately paint that
  // area ourselves (see the .status-bar-spacer in globals.css) instead of
  // our own pale --background color silently showing through behind the
  // system status bar, which is what washed out the status bar icons.
  viewportFit: 'cover',
};
    ...
    <html lang="en" translate="no" suppressHydrationWarning>
      ...
      <body className={cn(
          ...
          fontLogo.variable
            ...
            {/* Status bar spacer: paints a solid, correctly-contrasted block
                behind the OS status bar (env(safe-area-inset-top) is 0 on
                devices/browsers that don't draw edge-to-edge, so this is a
                no-op there). Without this, our pale --background color was
