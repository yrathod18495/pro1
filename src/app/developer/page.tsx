  ...
  return (
    <div className="min-h-screen bg-neutral-50 dark:bg-neutral-950 text-foreground flex flex-col">
      <main className="flex-1 py-8 px-4 md:px-8 max-w-7xl mx-auto space-y-8 w-full">
        ...
        <div className="relative overflow-hidden rounded-3xl border border-neutral-200/80 dark:border-neutral-800 bg-gradient-to-br from-white via-white to-primary/[0.03] dark:from-neutral-900 dark:via-neutral-900 dark:to-primary/[0.06] p-6 md:p-7 shadow-lg">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
            ...
            <div className="flex flex-wrap items-center gap-3">
              {keys.length === 0 && (
                <Button onClick={handleOpenDialog} size="lg" className="gap-2 font-bold shadow-md h-11 px-6 rounded-xl btn-shine">
                  <Plus className="w-4 h-4" /> Create New Secret Key
                </Button>
              )}
              {/* 🔴 NEW: logout used to fire immediately on one tap — on a
                  page whose whole purpose is managing production secret
                  keys, an accidental tap here is disruptive. A confirm
                  step makes the action actually deliberate. */}
              <AlertDialog>
                <AlertDialogTrigger asChild>
                  <Button variant="outline" size="lg" className="gap-2 font-semibold h-11 px-5 rounded-xl text-neutral-600 hover:text-red-600 hover:border-red-300 dark:text-neutral-400 dark:hover:text-red-400 transition-colors">
                ...
                <AlertDialogContent className="rounded-2xl">
                  <AlertDialogHeader>
                    <AlertDialogTitle className="flex items-center gap-2">
                      <LogOut className="w-4 h-4 text-red-500" /> Log out of Developer Center?
                    </AlertDialogTitle>
                    <AlertDialogDescription>
                      You'll be signed out of {user.email || 'this account'} and returned to the login page. Your secret keys and usage history stay exactly as they are — signing back in brings you right back here.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
                    <AlertDialogCancel className="rounded-xl">Cancel</AlertDialogCancel>
                    <AlertDialogAction onClick={() => logout()} className="rounded-xl bg-red-600 hover:bg-red-700 text-white gap-2">
          ...
          {/* Quick Stats Grid — each stat now its own card, not just a
              column of text, so the numbers that matter (credits, key
              count) actually stand out instead of blending into the row. */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-6 pt-6 border-t border-neutral-100 dark:border-neutral-800/80 relative z-10">
            <div className="space-y-1.5 rounded-2xl border border-neutral-200/70 dark:border-neutral-800 bg-white/60 dark:bg-neutral-950/30 p-3.5">
              <div className="flex items-center gap-1.5 text-neutral-400 dark:text-neutral-500">
                ...
                <p className="text-[10px] font-bold uppercase tracking-wider">Active Secret Keys</p>
              </div>
              <p className="text-2xl font-black text-neutral-900 dark:text-neutral-50">{keys.length}</p>
            </div>
            <div className="space-y-1.5 rounded-2xl border border-emerald-500/20 bg-emerald-500/[0.04] p-3.5">
            ...
            <Card className="border border-neutral-200/80 dark:border-neutral-800 bg-white dark:bg-neutral-900 shadow-sm rounded-2xl">
              ...
              <CardContent className="space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div className="flex items-center gap-3 border border-neutral-200/80 dark:border-neutral-800 rounded-xl p-3.5 bg-neutral-50/50 dark:bg-neutral-900/30">
                    <Badge className="bg-emerald-600 text-white font-mono text-xs px-2.5 py-0.5 font-bold rounded-md shrink-0">POST</Badge>
                    <code className="text-xs font-mono text-neutral-700 dark:text-neutral-300 truncate">/api/v1/analyze</code>
                    <span className="ml-auto text-[10px] text-neutral-400 shrink-0">detect characters — free</span>
                  </div>
                  <div className="flex items-center gap-3 border border-neutral-200/80 dark:border-neutral-800 rounded-xl p-3.5 bg-neutral-50/50 dark:bg-neutral-900/30">
                    <Badge className="bg-emerald-600 text-white font-mono text-xs px-2.5 py-0.5 font-bold rounded-md shrink-0">POST</Badge>
                    <code className="text-xs font-mono text-neutral-700 dark:text-neutral-300 truncate">/api/v1/generate</code>
                    <span className="ml-auto text-[10px] text-neutral-400 shrink-0">one-click full project</span>
                  </div>
                  <div className="flex items-center gap-3 border border-neutral-200/80 dark:border-neutral-800 rounded-xl p-3.5 bg-neutral-50/50 dark:bg-neutral-900/30 sm:col-span-2">
                    <Badge className="bg-blue-600 text-white font-mono text-xs px-2.5 py-0.5 font-bold rounded-md shrink-0">GET</Badge>
                    <code className="text-xs font-mono text-neutral-700 dark:text-neutral-300 truncate">/api/v1/generate/{'{project_id}'}</code>
                    <span className="ml-auto text-[10px] text-neutral-400 shrink-0">poll job status</span>
                  </div>
                </div>

                {/* 🔴 NEW: one-click full-project generation — script in,
                    finished audio out, either engine, priced at the exact
                    same per-character rate as the website (see
                    src/lib/pricing.ts). This is async: POST /generate
                    queues the job (same pipeline the /studio page's
                    Superfast mode uses) and returns a project_id right
                    away; poll GET /generate/{project_id} for the result. */}
                <div className="space-y-4 border border-neutral-200/80 dark:border-neutral-800 rounded-2xl p-5 bg-neutral-100/50 dark:bg-neutral-900/10">
                  <h3 className="font-bold text-sm flex items-center gap-2 text-neutral-900 dark:text-neutral-50">
                    <Sparkles className="w-4 h-4 text-primary" /> One-Click Full Project Generation
                  </h3>
                  <p className="text-xs text-neutral-500 dark:text-neutral-400 leading-relaxed">
                    Send a full script and get back finished, multi-character audio — either the Gemini or 11Labs engine, character detection and voice assignment handled automatically. Priced per character at the exact same rate shown on the website (Settings → Pricing), charged from the linked account's credit balance.
                  </p>

                  <div className="space-y-1.5">
                    <p className="text-[11px] font-bold text-neutral-500 uppercase tracking-wide">1. Analyze (free, no credits charged)</p>
                    <pre className="text-[11px] font-mono bg-neutral-950 text-neutral-100 rounded-xl p-3.5 overflow-x-auto">
{`curl -X POST "${baseUrl}/api/v1/analyze" \\
  -H "x-api-key: YOUR_API_KEY" \\
  -H "Content-Type: application/json" \\
  -d '{"script": "John: Hello there!\\nMary: Hi John!"}'`}
                    </pre>
                  </div>

                  <div className="space-y-1.5">
                    <p className="text-[11px] font-bold text-neutral-500 uppercase tracking-wide">2. Generate (charges credits, returns immediately with a project_id)</p>
                    <pre className="text-[11px] font-mono bg-neutral-950 text-neutral-100 rounded-xl p-3.5 overflow-x-auto">
{`curl -X POST "${baseUrl}/api/v1/generate" \\
  -H "x-api-key: YOUR_API_KEY" \\
  -H "Content-Type: application/json" \\
  -d '{"script": "John: Hello there!\\nMary: Hi John!", "engine": "elevenlabs"}'

# -> { "project_id": "HQ_...", "status": "in_queue", "estimated_cost": 42, "poll_url": "/api/v1/generate/HQ_..." }`}
                    </pre>
                  </div>

                  <div className="space-y-1.5">
                    <p className="text-[11px] font-bold text-neutral-500 uppercase tracking-wide">3. Poll for the result</p>
                    <pre className="text-[11px] font-mono bg-neutral-950 text-neutral-100 rounded-xl p-3.5 overflow-x-auto">
{`curl "${baseUrl}/api/v1/generate/HQ_..." -H "x-api-key: YOUR_API_KEY"

# -> { "status": "completed", "audio_url": "https://..." }   (or "in_queue" / "processing" / "error")`}
                    </pre>
                  </div>
                </div>

                {/* 🔴 FIX: this used to be ~120 lines of React state just
                    to test ONE endpoint (/api/v1/voice) — no way to test
                    /api/v1/analyze or /api/v1/generate at all from here.
                    Replaced with the same plain-HTML playground anyone can
                    also open standalone at /api-playground.html — it
                    tests every endpoint (analyze, generate + poll, voice
                    names) with zero React/build complexity, and embedding
                    it here vs. linking to it is just a convenience; the
                    tool itself is identical either way. */}
                <div className="space-y-3 border border-neutral-200/80 dark:border-neutral-800 rounded-2xl p-5 bg-neutral-100/50 dark:bg-neutral-900/10">
                  <div className="flex items-center justify-between">
                    <h3 className="font-bold text-sm flex items-center gap-2 text-neutral-900 dark:text-neutral-50">
                      <Play className="w-4 h-4 text-primary" /> Try It — Live API Playground
                    </h3>
                    <a
