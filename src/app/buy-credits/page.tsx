...
import { CheckCircle, Gem, Loader2, CreditCard, Tag, Coins, Briefcase, Rocket, Sparkles, TestTube, ShieldCheck, Trophy, Ticket, Clock, PartyPopper, AlertTriangle } from 'lucide-react';
...
export default function BuyCreditsPage() {
  ...
  const handleConfirmCreditRedeem = async () => {
    ...
    try {
        ...
        reportClientError('src/app/buy-credits/page.tsx:handleConfirmCreditRedeem', e);
        toast({ variant: 'destructive', title: 'Sync Error', description: 'Could not redeem promo code.' });
    } finally {
        setIsApplyingCode(false);
    }
  };
  ...
  const renderPlanCard = (plan: Plan) => {
    ...
    return (
      <Card key={plan.id} className={cn(
        "relative flex flex-col w-full max-w-md mx-auto h-full transition-all duration-500 rounded-[2.5rem] overflow-visible group border-border/50 hover:border-primary/50 shadow-sm hover:shadow-2xl",
        ...
        plan.bestValue && "border-2 border-amber-400/80 shadow-2xl shadow-amber-500/20 hover:shadow-amber-500/30 lg:-translate-y-3 bg-gradient-to-b from-amber-50/60 via-white to-white dark:from-amber-950/10 dark:via-background dark:to-background"
      )}>
        {plan.bestValue && (
          <div className="absolute -top-4 inset-x-0 flex justify-center z-20">
            <div className="flex items-center gap-1.5 bg-gradient-to-r from-amber-500 to-orange-500 text-white text-[10px] font-black uppercase tracking-widest px-5 py-2 rounded-full shadow-lg shadow-amber-500/40 border-2 border-white dark:border-background">
    ...
    <>
      <div className="container mx-auto max-w-7xl py-12 px-4 animate-in fade-in duration-700">
          ...
          {user && (
            <div className="pt-2 flex justify-center">
              <div className="inline-flex items-center gap-2.5 px-6 py-2.5 rounded-2xl bg-primary/10 border border-primary/20 text-primary shadow-sm">
                ...
                <span className="text-sm font-black tracking-tight text-primary">{formatCredits(user.credits)} Credits</span>
              </div>
            </div>
          )}
        </div>
        ...
        <section className="mb-20">
            <div className="text-center mb-10 px-2 space-y-1.5">
