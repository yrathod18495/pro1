...
async function createRazorpayOrder(
    ...
    reportServerError('src/app/buy-credits/actions.ts#1', error, { userId: notes.userId || 'unknown' });
...
export async function handlePurchaseAction(
  ...
  try {
    ...
    reportServerError('src/app/buy-credits/actions.ts#2', error, { userEmail: user.email || 'unknown', userId: user.uid });
...
export async function handleCustomTopupAction(
    ...
    try {
        ...
        reportServerError('src/app/buy-credits/actions.ts#topup', error, { userEmail: user.email || 'unknown', userId: user.uid });
