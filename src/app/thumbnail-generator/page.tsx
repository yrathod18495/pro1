...
export default function ThumbnailGeneratorPage() {
  ...
  // 🔴 Tracks which mappingIds the RTDB `tempThumbnailGenerations` node
  // reported last time, so the merge below can tell "this job is gone
  // from RTDB now" apart from "RTDB just hasn't sent an update yet" —
  // see the fix comment in the RTDB listener for why this is needed.
  const rtdbSeenIdsRef = useRef<Set<string>>(new Set());
    ...
    if (database) {
      try {
        ...
        unsubRtdb = onRtdbValue(rtdbRef, (snapshot) => {
          // 🔴 FIX: this used to `return` here when the node was empty —
          // which is exactly the state right after a delete. That meant
          // deleting a job removed it from RTDB but never told this
          // component to remove it from `recentJobs`, so it stayed
          // visible ("Processing" forever) until a full page reload.
          ...
          // Which mappingIds RTDB reported LAST time vs THIS time — the
          // difference is what got deleted from RTDB since the last event.
          const previouslySeen = rtdbSeenIdsRef.current;
          const nowSeen = new Set(incoming.map((j) => j.mappingId));
          const removedFromRtdb = [...previouslySeen].filter((id) => !nowSeen.has(id));
          rtdbSeenIdsRef.current = nowSeen;

          if (incoming.length > 0 || removedFromRtdb.length > 0) {
            setRecentJobs((prev) => {
              const map = new Map<string, ActiveThumbnailJob>();
              prev.forEach((p) => map.set(p.mappingId, p));
              // 🔴 Actually remove jobs RTDB no longer has — previously
              // this map only ever grew (upsert-only union), so a
              // deleted job's stale copy lived in `recentJobs` forever
              // even after a full RTDB resync.
              removedFromRtdb.forEach((id) => map.delete(id));
              incoming.forEach((j) => {
                const existing = map.get(j.mappingId);
                if (!existing) {
                  map.set(j.mappingId, j);
