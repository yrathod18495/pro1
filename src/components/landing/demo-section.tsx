...
import { Music, Play, MonitorPlay, Film, Languages, Gauge } from 'lucide-react';
    ...
    return (
        ...
        >
            <Card className="group relative overflow-hidden rounded-[2.5rem] border-primary/5 bg-card hover:border-primary/20 transition-all duration-500 hover:shadow-2xl hover:-translate-y-1">
                <div className="absolute -bottom-6 -right-6 select-none pointer-events-none opacity-[0.05] dark:opacity-[0.08] group-hover:opacity-[0.15] transition-all duration-700 scale-100 group-hover:scale-110">
    ...
    return (
        <section id="demo" className="w-full py-16 md:py-24 overflow-hidden relative bg-muted/5">
            ...
            <div className="container relative z-10 px-4 md:px-6 text-center">
                <div className="space-y-5 mb-12 animate-in fade-in slide-in-from-top-4 duration-1000">
                ...
                <div className="mt-16 pt-10 border-t border-border/50 flex flex-wrap items-center justify-center gap-x-10 gap-y-4">
                    {[
                        { icon: Film, label: 'Cinematic HDR' },
                        { icon: Languages, label: 'Neural dubbing' },
                        { icon: Gauge, label: 'Ultra-low latency' },
                    ].map(({ icon: Icon, label }) => (
                        <div key={label} className="flex items-center gap-2 text-sm font-semibold text-muted-foreground anim-surface-tint px-3 py-1.5 rounded-full">
