...
interface PricingConfig {
  ...
  // 11Labs is billed separately from Gemini — it costs us real ElevenLabs
  // characters, so it gets its own multiplier rather than reusing studioNormal.
  elevenLabsNormal: number;
  elevenLabsDiscounted: number;
...
const DEFAULT_PRICING: PricingConfig = {
  ...
  elevenLabsNormal: 2.5,
  elevenLabsDiscounted: 2.0,
  script10Normal: 1000,
...
export function PricingSettingsManager() {
    ...
    const unsubscribe = onRtdbValue(pricingRef, (snapshot) => {
      if (snapshot.exists()) {
        ...
        setPricing({
          ...
          elevenLabsNormal: Number(data.elevenLabsNormal ?? DEFAULT_PRICING.elevenLabsNormal),
          elevenLabsDiscounted: Number(data.elevenLabsDiscounted ?? DEFAULT_PRICING.elevenLabsDiscounted),
  ...
  const handleInputChange = (key: keyof PricingConfig, value: string) => {
    ...
    setPricing(prev => {
      ...
      if (key === 'elevenLabsNormal') {
        next.elevenLabsDiscounted = numVal > 0 ? parseFloat((numVal * 0.8).toFixed(2)) : 0;
      }
      if (key === 'studioNormal') {
  ...
  return (
    <div className="space-y-8 pb-20">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        {/* 11Labs — priced separately from Gemini. The studio charges
            whichever of these two multipliers matches the engine the user
            picked, and server-files/11.py bills against the same figure. */}
        <Card className="rounded-3xl border-none shadow-xl bg-card overflow-hidden">
          <CardHeader className="bg-violet-500/5 border-b border-violet-500/10 p-5 flex flex-row items-center justify-between">
            <CardTitle className="text-[10px] font-black uppercase tracking-widest flex items-center gap-2">
              <MicVocal className="h-5 w-5 text-violet-500" />
              Studio 11Labs
            </CardTitle>
            {renderDiscountBadge(pricing.elevenLabsNormal, pricing.elevenLabsDiscounted)}
          </CardHeader>
          <CardContent className="p-6 space-y-4">
            <p className="text-[9px] font-bold text-muted-foreground uppercase tracking-wider mb-2">Cost per character multiplier</p>
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  step="0.1"
                  min="0"
                  value={pricing.elevenLabsNormal || ''}
                  onChange={(e) => handleInputChange('elevenLabsNormal', e.target.value)}
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  step="0.1"
                  min="0"
                  value={pricing.elevenLabsDiscounted || ''}
                  onChange={(e) => handleInputChange('elevenLabsDiscounted', e.target.value)}
                  className="h-11 rounded-2xl bg-muted/10 border-violet-500/10 font-mono text-sm px-4"
                />
              </div>
            </div>
