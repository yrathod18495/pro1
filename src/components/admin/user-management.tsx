...
import { 
    Users, MoreHorizontal, Search, Loader2, Coins, ShieldCheck, Send, 
...
function UserUnifiedViewDialog({ 
    ...
    const [isDownloadingViewingPdf, setIsDownloadingViewingPdf] = useState(false);
    const [viewingScriptText, setViewingScriptText] = useState('');
    const [isLoadingFullScript, setIsLoadingFullScript] = useState(false);
    ...
    const fetchProjectsTab = useCallback(async () => {
        ...
        try {
            ...
            // 🐛 FIX: AI Script Studio saves its results under a dedicated
            // script_projects/{userId}/userProjects collection (see
            // server-files/script_generation.py) — this was never queried,
            // so script-generator projects silently never showed up here.
            let scriptPartitionedProjs: any[] = [];
            try {
                const scriptPartitionedQuery = query(
                    collection(firestore, 'script_projects', user.uid, 'userProjects'),
                    limit(projectsLimit + 5)
                );
                const scriptPartitionedSnap = await getDocs(scriptPartitionedQuery);
                scriptPartitionedProjs = scriptPartitionedSnap.docs.map(d => ({
                    ...d.data(),
                    id: d.id,
                    ref: d.ref,
                    studioType: 'script',
    ...
    // 🐛 FIX: AI Script Studio never writes the full script into the
    // Firestore `script` field — only a ~300-char `teaser` is stored there,
    // the complete text lives at `scriptUrl` in R2 (see
    // server-files/script_generation.py). This modal used to read
    // `viewingProject.script` directly everywhere (preview AND all three
    // downloads), so both the preview and every export silently showed/saved
    // only the short teaser instead of the real script. Mirrors the
    // `fetchFullScript()` pattern already used correctly in
    // src/app/history/project-card.tsx.
    const fetchFullViewingScript = async (project: Project): Promise<string> => {
        const scriptUrl = (project as any)?.scriptUrl;
        if (scriptUrl) {
            try {
                const url = getDisplayUrl(scriptUrl);
        ...
        if (typeof (project as any).teaser === 'string' && (project as any).teaser) return (project as any).teaser;
    ...
    useEffect(() => {
        if (!viewingProject) {
            setViewingScriptText('');
            return;
        }
        let cancelled = false;
        setIsLoadingFullScript(true);
        fetchFullViewingScript(viewingProject).then(text => {
            if (!cancelled) {
                setViewingScriptText(text);
                setIsLoadingFullScript(false);
            }
        });
    ...
    const handleCopyScript = () => {
        if (!viewingScriptText) return;
        navigator.clipboard.writeText(viewingScriptText);
        setIsCopied(true);
    ...
    const handleDownloadScriptTxt = () => {
        if (!viewingScriptText) {
            toast({ variant: 'destructive', title: 'No script content available' });
            return;
        }
        downloadScriptAsTxt(viewingProject?.projectName || 'Script', viewingScriptText);
    ...
    const handleDownloadScriptDocx = async () => {
        ...
        try {
            await downloadScriptAsDocx(viewingProject?.projectName || 'Script', viewingScriptText);
            toast({ title: 'Downloaded .DOCX' });
        } catch (err) {
            reportClientError('src/components/admin/user-management.tsx:handleDownloadScriptDocx', err);
    ...
    const handleDownloadScriptPdf = async () => {
        ...
        setIsDownloadingViewingPdf(true);
        try {
            await downloadScriptAsPdf(viewingProject?.projectName || 'Script', viewingScriptText);
            toast({ title: 'Downloaded .PDF' });
        } catch (err) {
            reportClientError('src/components/admin/user-management.tsx:handleDownloadScriptPdf', err);
            toast({ variant: 'destructive', title: 'PDF generation failed' });
        } finally {
            setIsDownloadingViewingPdf(false);
        }
    };
...
export function UserManagement() {
  ...
  // Direct message target — held as plain row data rather than a fetched
  // profile, since the dialog only needs uid/name/email.
  const [showReachOut, setShowReachOut] = useState(false);
  const [reachOutTarget, setReachOutTarget] = useState<{ id?: string; uid?: string; name?: string; email?: string } | null>(null);
  ...
  return (
    <div className="space-y-8 pb-20">
                    ...
                    {activeConsistencyUsers ? (
                        ...
                        return (
                            <TableRow key={user.uid} className="h-24 hover:bg-primary/5 transition-colors border-muted/10 group">
                                ...
                                <TableCell className="pr-8 text-right">
                                    <div className="flex items-center justify-end gap-2">
                                        <Button size="sm" variant="outline" onClick={() => fetchProfile(user.uid).then(p => { if(p) { setSelectedProfile(p); setShowUnifiedView(true); } }).catch((err) => console.error("[UserManagement] fetchProfile failed:", err))} className="h-9 px-5 rounded-full font-black text-[9px] uppercase tracking-widest border-primary/20">Analyze</Button>
                                        <DropdownMenu>
                                            ...
                                            <DropdownMenuContent align="end" className="w-56 p-2 rounded-xl shadow-2xl border-primary/5">
                                                ...
                                                <DropdownMenuItem className="h-10 rounded-lg cursor-pointer" onClick={() => { setReachOutTarget({ id: user.uid, uid: user.uid, name: user.name, email: user.email }); setShowReachOut(true); }}>
                                                    <Send className="mr-3 h-4 w-4 text-primary" /> Send Message
                                                </DropdownMenuItem>
                                            </DropdownMenuContent>
                                        </DropdownMenu>
                                    </div>
      ...
      <UserReachOutDialog
        open={showReachOut}
        onOpenChange={(open) => { setShowReachOut(open); if (!open) setReachOutTarget(null); }}
        user={reachOutTarget}
      />
