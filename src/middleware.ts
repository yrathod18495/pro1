...
export function middleware(request: NextRequest) {
  ...
  //
  // Same problem applies to RSC client-side navigations/prefetches (clicking
  // a <Link>, router.push, or Next's automatic prefetch): these are GET
  // requests too, but the client expects a flight/RSC payload back, not a
  // 301 + full HTML page. If we redirect those, the browser's fetch follows
  // the redirect, lands on a normal HTML response, and Next's router throws
  // the exact same "unexpected response" error. Next tags these requests
  // with an 'RSC' header (and prefetches also carry 'Next-Router-Prefetch'),
  // so they must be excluded from this redirect the same way Server Actions are.
  ...
  if (host === '12labs.in' && isNavigation && !isServerAction && !isRscRequest) {
