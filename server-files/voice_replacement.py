...
def normalize_name(n):
    ...
    # Same multi-language narrator recognition as studio.py's normalize_name
...
# FIX (RTDB bandwidth bug): finished voice_replacement jobs were never
# removed from RTDB, so the node grew forever and was re-downloaded in full
# every 5 seconds by the poll safety-net (_drain_pending_replacements). This
# deletes a job a short while after it reaches a terminal state, giving the
# frontend's realtime listener time to receive the final status first.
_VR_JOB_CLEANUP_DELAY_SECONDS = 30

def _cleanup_voice_replacement_job_later(request_id, delay=_VR_JOB_CLEANUP_DELAY_SECONDS):
    def _do_cleanup():
        time.sleep(delay)
        try:
            db.reference(f"voice_replacement/{request_id}").delete()
        except Exception as cleanup_err:
            log_error(f"Failed to clean up finished voice_replacement job {request_id}", str(cleanup_err))
...
def process_voice_replacement(request_id, data):
    ...
    try:
        ...
        _cleanup_voice_replacement_job_later(request_id)
...
# --- 🔊 LISTENER (event-driven — reacts only to the entry that actually
# changed, never re-reads the whole node) ---
REQUIRED_FIELDS = ("projectId", "userId", "originalAudioUrl")
WRITE_GRACE_SECONDS = 3  # if the entry looks incomplete but was just written
                          # (e.g. frontend sets fields in separate calls),
                          # wait for the next event instead of failing early.
...
def _maybe_start_replacement(rid, data):
    """Starts a job only if it's actually pending, not already running, and
    (if newly-written) fully written. Never reads anything from RTDB itself
    — `data` is whatever the listener event already delivered."""
    ...
    _check_job_abuse(data.get("userId"), "voice_replacement")
    threading.Thread(target=process_voice_replacement, args=(rid, data), daemon=True).start()


def _on_voice_replacement_event(event):
    """Callback for db.reference('voice_replacement').listen(...).
    IMPORTANT: this used to call a full `.get()` of the whole
    `voice_replacement` node on every single event (including our OWN
    status writes like 'processing'/'completed'/'error') — meaning every
    status transition re-downloaded every job ever sitting in that node,
    finished or not. That was a major, ongoing source of RTDB bandwidth
    (same root cause as the old 5-second polls elsewhere). Fixed to use
    ONLY the data the event itself already carries:
      path == "/"           -> full snapshot on first attach: {requestId: {...}}
      path == "/{requestId}" -> one request created/replaced
      path == "/{requestId}/{field}" -> a single field changed (almost
                                         always our own status/result
                                         write) -> deliberately ignored,
                                         so we never re-process or re-fetch
                                         anything because of our own write.
    """
    ...
    try:
        ...
        log_error("voice_replacement listener callback error", str(e))
...
#
# NOTE: there used to be a 5-second poll loop here that re-scanned the
# whole node on a timer to compensate. It's removed on purpose — it was
# also what re-downloaded this node in full every 5s, contributing to the
# runaway RTDB bandwidth bill. Pickup now runs purely off `.listen()`
# state; nothing compensates on a timer if the stream goes silent.
...
def _attach_voice_replacement_listener():
    try:
        ...
        _listener_last_event_ts[0] = time.time()
        log_success("👂 Realtime listener attached: voice_replacement")
    except Exception as e:
...
def start_pending_voice_replacement_listener():
    ...
    `.listen()`'s own first event already delivers a full snapshot (path
    "/"), so a second full read here would just be the same download twice
    on every process start. No polling loop either. Safe to call more than
    once — it's guarded by app.py's startup lifespan already, but this
    itself has no re-entrancy guard, so don't call it twice manually."""
    ...
    log_success("Voice-replacement worker ready: realtime listener only (no poll, no duplicate startup read)")
