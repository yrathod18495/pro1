...
# ------------------------------------------------------------------ #
TEXT_MODELS = ["gemini-3.8-flash", "gemini-3.6-flash", "gemini-3.5-flash"]
...
class ModelCallError(Exception):
    ...
    (when known) so callers can tell a model-wide outage apart from a
...
#   503 UNAVAILABLE — the model is under high demand for EVERYONE. It has
#       nothing to do with which key you hold, so rotating through 32 keys
#       just wastes ~30s each (that's what the STORY_...834NXE log shows:
#       key #1..#25 all failing 503 on the same model for minutes) and
#       delays reaching a model that actually works. Skip the model at once.
...
def call_gemini_model(model_name, api_key, prompt, generation_config, timeout=240):
    ...
    if not res.ok:
        raise ModelCallError(
            f"[{model_name}] Gemini API Rejection ({res.status_code}): {res.text[:300]}",
...
def call_gemini_with_fallback(primary_model, fallback_chain, prompt, generation_config, context=""):
    ...
    giving up on that model and moving to the next one — EXCEPT when the
    model answers 503 (see MODEL_UNAVAILABLE_STATUS_CODES), which means the
    model is overloaded for everyone; then it skips the remaining keys and
    jumps to the next model immediately. A 429 still rotates keys, since
    that one is a per-key quota limit another key can satisfy.
        ...
        for key_idx, api_key in enumerate(GEMINI_KEYS):
            try:
                ...
                if status in MODEL_UNAVAILABLE_STATUS_CODES:
                    # 503: the model is overloaded for everyone, not just this
                    # key. Burning the remaining keys on it only costs time
                    # (~30s per attempt) — jump straight to the next model.
                    log_warn(f"{tag}⚠️ {model_name} is overloaded ({status}) — model-wide, not a key problem. Skipping remaining key(s) and moving to the next model right away.")
                    break
                continue
        else:
...
def build_language_directive(language):
    ...
    return (
        ...
        + build_grammatical_gender_directive(language)
    )


def build_grammatical_gender_directive(language):
    """Hindi (and Urdu, Marathi, Gujarati, Punjabi, Bengali...) inflect the
    VERB for the speaker's gender: a man says करूंगा / गया / रहा था, a woman
    says करूंगी / गई / रही थी. Nothing in the prompt used to mention this, so
    the model picked endings at random and a female character would say
    "मैं करूंगा" — an error every native speaker hears instantly.

    This spells the rule out and, importantly, tells the model to FIX the
    gender of each character before writing their first line, so the whole
    script stays consistent rather than drifting halfway through."""
    return (
        "\n\nGRAMMATICAL GENDER — STRICT (this is the most commonly botched "
        "part of the output, so check it on every single line):\n"
        "- Many languages inflect verbs, adjectives and participles for the "
        "SPEAKER's gender. Hindi is the clearest case: a male speaker says "
        "मैं करूंगा / मैं गया / मैं जा रहा था / मैं खुश हूँ (खुश is invariant) — a female "
        "speaker says मैं करूंगी / मैं गई / मैं जा रही थी. The same applies to Urdu, "
        "Marathi, Gujarati, Punjabi and other gender-marking languages.\n"
        "- Decide each character's gender ONCE, before you write their first "
        "line, and keep every verb ending consistent with it for the entire "
        "script. Never let a character switch endings partway through.\n"
        "- The agreement follows the SPEAKER when they talk about themselves, "
        "and the SUBJECT being described otherwise. A male character speaking "
        "ABOUT a woman still uses her feminine forms: वह चली गई, not वह चला गया.\n"
        "- This also covers the narrator: pick the narrator's gender and hold "
        "it steady in any line where the narrator refers to themselves.\n"
        "- If the target language does not mark gender on verbs (English, "
        "Tamil, Telugu, Kannada, Malayalam, Indonesian...), ignore this "
        "section entirely — do not invent gendered forms that don't exist."
    )
